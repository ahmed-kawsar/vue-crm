<template>
  <div>
    <h1 class="text-3xl font-bold text-gray-800 mb-5">Leads</h1>
    ...
    <div class="overflow-x-auto">
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="(lead, index) in paginatedLeads" :key="index">
    ...
    <div v-if="isCreating" class="fixed z-10 inset-0 overflow-y-auto"></div>
...
const newLead = ref({ name: "", email: "" });
const editedLead = ref({ id: null, name: "", email: "" });

const totalPages = computed(() => Math.ceil(leads.value.length / perPage));
...
const createLead = () => {
  const newId =
    leads.value.length > 0
      ? Math.max(...leads.value.map((lead) => lead.id)) + 1
      : 1;
  leads.value.unshift({
    id: newId,
  ...
  newLead.value = { name: "", email: "" };
  isCreating.value = false;
};

const editLead = (id) => {
  const leadToEdit = leads.value.find((lead) => lead.id === id);
  if (leadToEdit) {
    editedLead.value = { ...leadToEdit };
    isEditing.value = true;
  }
};

const saveEdit = () => {
  const leadIndex = leads.value.findIndex(
    (lead) => lead.id === editedLead.value.id
  );
  if (leadIndex !== -1) {
    leads.value.splice(leadIndex, 1, { ...editedLead.value });
